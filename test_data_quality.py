...
def test_no_empty_texts():
    for split in dataset.keys():
        ...
        ), f"Found empty text in {split} split."
...
def test_text_length():
    ...
    for split in dataset.keys():
        texts = dataset[split]["text"]
        assert all(
            len(text.split()) <= max_length for text in texts
        ), f"Found text longer than {max_length} tokens in {split} split."
...
def test_label_distribution():
    for split in dataset.keys():
        ...
        ), f"Found label with zero instances in {split} split."
